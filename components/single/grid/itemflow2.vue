<template>
	<view id="flow">
		<view class="flow-content">
			<view class="steps-item" v-for="(item,index) in datas" :key="index" v-if="item.broadcast.length>0">
				...
				<view class="steps-content">
					<view class="journal-item" v-for="(broadcast,bindex) in item.broadcast" :key="bindex">
						<view class="journal-item-info">
							<view class="journal-title">
								<image :lazy-load="true" class="loading journal-avatar" :src="broadcast.author.avatar==null?basePhoto:broadcast.author.avatar"></image>
								<view class="journal-title-desc">
									<view class="journal-author">{{broadcast.author.name==null?"智家视":broadcast.author.name}}</view>
	...
	export default {
		...
		methods: {
			...
			onDel(e) {
				...
				var that = this;
				this.$axios.post(this.umcons.serviceAddress.DEL_BROADCAST, params, function(res, code) {
					console.log(res)
					if (code == 0) {
