<template>
	<view class="calendar-boxs">
		...
		<view v-if="maskShow || insert" :class="{ 'ani-calendar-show': aniMaskShow, 'uni-calendar__static': insert }" class="uni-calendar__box">
			...
			<view class="bg-img" style="background-image: url('https://wxt.homeeyes.cn/app/images/bgcalendar.png'); background-size: 125% 1000upx;">
...
<script>
	...
	import uniCalendarItem from './uni-calendar-item-green.vue'
	export default {
		name: 'UniCalendar',
		components: {
			uniCalendarItem
		...
		methods: {
			...
			dataBefor(id, types) {
				...
				let nowSelect = f(this.canlender.year) + '-' + f(this.canlender.month)
